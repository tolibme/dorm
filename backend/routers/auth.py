...
def login(request: Request, payload: LoginRequest, db: Session = Depends(get_db)):
    ...
    if student and student.password_hash and verify_password(payload.password, student.password_hash):
        ...
        return TokenResponse(access_token=token, role="student", user_id=student.student_id, name=student.full_name)
    ...
    if staff and staff.password_hash and verify_password(payload.password, staff.password_hash):
        ...
        return TokenResponse(access_token=token, role="staff", user_id=staff.staff_id, name=staff.full_name)
